...
# Button-1> = Left mouse button, 2->middle
...
# Add these buttons to your UI setup
seek_frame = ttk.Frame(root)
seek_frame.pack(pady=5)

backward_btn = ttk.Button(seek_frame, text="⏪ -5s", command=lambda: seek_video(-5))
backward_btn.pack(side='left', padx=5)

forward_btn = ttk.Button(seek_frame, text="⏩ +5s", command=lambda: seek_video(5))
forward_btn.pack(side='left', padx=5)

# Add seeking function
def seek_video(seconds):
    current_time1 = video1.current_duration()
    current_time2 = video2.current_duration()
    
    # Seek both videos
    new_time1 = max(0, current_time1 + seconds)
    new_time2 = max(0, current_time2 + seconds)
    
    video1.seek(new_time1)
    video2.seek(new_time2)
